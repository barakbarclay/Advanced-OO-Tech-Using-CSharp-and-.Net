...
namespace LINQProblems
{
    public class Program
    {
        ...
        public static Item DetermineWhichItemWasMostProfitable()
        {
            ...
            highestEarner = items.Aggregate((i1, i2) => (i1.NumberOfSales * i1.ItemPrice) > (i2.NumberOfSales * i2.ItemPrice) ? i1 : i2);
        ...
        public static List<FileInfo> GetAllPNG(string path)
        {
            ...
            IEnumerable<FileInfo> ifiles = new List<FileInfo>();
            IEnumerable<DirectoryInfo> directories = new List<DirectoryInfo>();

            directories = new DirectoryInfo(path).EnumerateDirectories("*", SearchOption.AllDirectories);
            directories.ToList().ForEach(t => files.AddRange(GetAllPNG(t.FullName)));
            //directories.ToList().ForEach(t => ifiles = ifiles.Concat(t.EnumerateFiles("*.png")));
            //files = ifiles.ToList();
        ...
        /// </summary>
        public delegate int OneIntParam(int x);
        public static List<int> GetFibonacciNumbers()
        {
            ...
            input.ToList().ForEach(t => results.Add(Fibonacci(t)));

            return results;
            input.ToList().ForEach(t => results.Add(Fibonacci(t)));

            OneIntParam FibonacciLambda = (t) =>
            {
                if (t == 1 || t == 2)
                    return 1;
                return FibonacciLambda(t - 1) + Fibonacci(t - 2);
            };
        ...
        public static int SumPrimes()
        {
            ...
            IEnumerable<int> primes = Enumerable.Range(2, 1000)
